...
function resolvePositioning(tooltip, { at, adjustment }, usePosition, center) {
  const isAbove = usePosition === PLACE_ABOVE;
  ...
  top = (isAbove ? rect.top - rect.height : top) || undefined;
...
function Tooltip({
  ...
  function handleEnter() {
    ...
    const positionOffset = usePosition
      ? clientX
      : center
      ? rect.left + halfWidth
      : rect.left;

    const space = window.innerWidth - positionOffset - scrollOffset;
    ...
    if (tooFarLeft) {
      ...
      adjustment = -rect.left + 5 - delta;
    } else if (tooFarRight && center) {
      const protrution = halfWidth + rect.width / 2;
      const widthDelta = space - protrution;

      adjustment = widthDelta - scrollOffset - 5;
    } else if (tooFarRight) {
      const protrution = contentWidth - react.width;

      adjustment = space - protrution;
    }
