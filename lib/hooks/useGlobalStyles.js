...
export function useGlobalStyles(opts = {}) {
  const includeGlobalBase = opts.includeGlobalBase ?? true;
  const includeFont = opts.includeFont ?? true;
  const includeFloatLabel = opts.includeFloatLabel ?? true;
  const includeRipple = opts.includeRipple ?? true;

  useEffect(() => {
    const injectStyles = [];

    if (includeGlobalBase) injectStyles.push(globalBase);
    if (includeFont) injectStyles.push(fontBase);
    if (includeFloatLabel) injectStyles.push(floatLabel);
    if (includeRipple) injectStyles.push(ripple);

    const tag = document.createElement('style');
    tag.setAttribute('data-mko-global-styles', '');
    tag.textContent = injectStyles.join('\r\n');

    document.head.appendChild(tag);
  }, [includeGlobalBase, includeFont, includeFloatLabel, includeRipple]);
}
