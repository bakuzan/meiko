...
export const dateIsOutOfRange = (state, option, { afterDate, beforeDate }) => {
  ...
  if (isMonthView) {
    ...
    date.setDate(value);
    return (
      (afterDate && DateUtils.isBefore(date, afterDate)) ||
      (beforeDate && !DateUtils.isBefore(date, beforeDate))
    );
  } else {
    ...
    const lastOfMonth = DateUtils.getLastDateOfMonth(d);
    return (
      (afterDate && DateUtils.isBefore(lastOfMonth, afterDate)) ||
      (beforeDate && !DateUtils.isBefore(firstOfMonth, beforeDate))
    );
  }
};
