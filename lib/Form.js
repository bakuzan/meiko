...
class Form extends React.Component {
  ...
  render() {
    ...
    return (
      <div
        className={classNames('mko-form', styles.mkoForm, className)}
        {...props}
      >
        {hasTitle && (
          <h2 className={classNames('mko-form__title', styles.title)}>
            {title}
          </h2>
        )}
        <form
...
Form.defaultProps = {
  submitOptions: { hide: false },
  cancelOptions: { hide: false }
};
Form.propTypes = {
  ...
  cancelOptions: PropTypes.shape({ onCancel: PropTypes.func })
};
