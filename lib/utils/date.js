...
export const getDayName = date => Strings.dayNames[new Date(date).getDay()];
...
export const adjustDateMonth = (date, adjustment) => {
  const d = new Date(date);
  return new Date(d.getFullYear(), d.getMonth() + adjustment, d.getDate());
};
export const adjustDateYear = (date, adjustment) => {
  const d = new Date(date);
...
export const isBefore = (d1, d2) => {
  const dx = new Date(d1).getTime();
  const dy = new Date(d2).getTime();
  return dx < dy;
};
